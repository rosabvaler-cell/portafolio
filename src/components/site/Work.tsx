...
export const Work = () => {
  return (
    <section id="trabajo" className="relative overflow-hidden bg-[hsl(220_38%_6%)] text-cream">
      {/* Soft red glows — layered for a smooth diffuse */}
      <div
        aria-hidden
        className="pointer-events-none absolute -top-40 left-1/2 -translate-x-1/2 h-[80vh] w-[120vw]"
        style={{
          background:
            "radial-gradient(ellipse 50% 60% at 50% 50%, hsl(var(--secondary) / 0.55) 0%, hsl(var(--secondary) / 0.2) 40%, transparent 70%)",
          filter: "blur(80px)",
        }}
      />
      <div
        aria-hidden
        className="pointer-events-none absolute -bottom-40 left-1/2 -translate-x-1/2 h-[70vh] w-[120vw]"
        style={{
          background:
            "radial-gradient(ellipse 55% 60% at 50% 50%, hsl(var(--secondary) / 0.45) 0%, transparent 70%)",
          filter: "blur(100px)",
        }}
      />
      {/* Vignette to fade edges into pure black */}
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0"
        style={{
          background:
            "radial-gradient(ellipse 90% 80% at 50% 50%, transparent 40%, hsl(220 38% 6%) 95%)",
        }}
      />
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 top-32 flex items-center justify-center select-none"
      >
        <span className="font-display uppercase text-[18vw] leading-none tracking-tight text-white/[0.035] whitespace-nowrap">
          TRABAJO
        </span>
      </div>

      {/* Header */}
      <div className="container relative z-10 pt-24 md:pt-32 pb-20 md:pb-28 text-center">
        <div className="mx-auto mb-10 flex items-center justify-center gap-3 opacity-70">
          <span className="h-px w-24 bg-secondary/60" />
          <span className="h-1.5 w-1.5 rotate-45 bg-secondary" />
          <span className="h-px w-24 bg-secondary/60" />
        </div>


        <h2 className="reveal mt-10 font-serif italic leading-[0.95] text-white whitespace-nowrap text-[clamp(2rem,8vw,7rem)]">
          Así se ve nuestro{" "}
          <span className="not-italic font-serif text-secondary">trabajo</span>
        </h2>


        <p className="reveal mx-auto mt-10 max-w-xl text-white/60 leading-relaxed">
          Descubre cómo adaptamos nuestra creatividad y estrategia a distintos
          rubros — sin fórmulas repetidas, cada marca con su propio lenguaje.
        </p>
      </div>

      {/* Grid */}
      <div className="container relative z-10 pb-28 md:pb-40">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-10 max-w-5xl mx-auto">
          {industries.map((it, i) => (
            ...
            >
              <p
                className={`font-script text-white text-4xl md:text-5xl lg:text-6xl leading-none ${
                  it.align === "left" ? "text-left" : "text-right"
                }`}
              >
                {it.label}
              </p>
              <article className="group relative aspect-[9/16] overflow-hidden bg-[hsl(220_38%_12%)] ring-1 ring-white/5 shadow-[0_30px_80px_-30px_hsl(var(--secondary)/0.6)] cursor-pointer">
                {it.videos && it.videos.length > 0 ? (
                  ...
                  <div
                    ...
                    style={{
                      backgroundImage:
                        "linear-gradient(135deg, hsl(220 38% 12%) 0%, hsl(220 38% 8%) 50%, hsl(var(--secondary) / 0.85) 100%)",
                    }}
                  />
                )}
              </article>
